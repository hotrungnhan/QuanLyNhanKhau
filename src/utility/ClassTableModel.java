...
//        });
    ...
    public DefaultTableModel setTableTamVang(List<TamVangModel> listItem, String[] listColumn) {
        final int columns = listColumn.length;
        DefaultTableModel dtm = new DefaultTableModel() {
            @Override
            public boolean isCellEditable(int row, int column) {
        ...
        listItem.forEach((item) -> {
            obj[0] = item.getMaGiayTamVang();
            obj[1] = item.getNoiTamTru();
    ...
    public DefaultTableModel setTableTamTru(List<TamTruModel> listItem, String[] listColumn) {
        final int columns = listColumn.length;
        DefaultTableModel dtm = new DefaultTableModel() {
            @Override
            public boolean isCellEditable(int row, int column) {
        ...
        obj = new Object[columns];
        listItem.forEach((item) -> {
            obj[0] = item.getMaGiayTamTru();
            obj[1] = item.getSoDienThoaiNguoiDangKy();
            obj[2] = item.getTuNgay();
            obj[3] = item.getDenNgay();
            obj[4] = item.getLyDo();
            dtm.addRow(obj);
